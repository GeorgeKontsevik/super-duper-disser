...
$$
\begin{array}{lcl}
x_i^V(t) & = & (r_i,\ d_i^k(t),\ q_i^k(t)),\\
r_i & = & \text{роль узла: спрос / предложение / mixed},\\
d_i^k(t) & = & \text{величина спроса типа } k,\\
q_i^k(t) & = & \text{мощность предложения / сервиса типа } k.
\end{array}
$$
...
$$
\begin{array}{lcl}
x_e^E(t) & = & (\kappa_e,\rho_e,l_e,a_e(t),c_e(t)),\\
\kappa_e & = & \text{класс / тип link},\\
\rho_e & = & \text{качество / поверхность / режим link},\\
l_e & = & \text{длина link},\\
a_e(t) & = & \text{доступность link},\\
c_e(t) & = & \text{время / стоимость прохождения link}.
\end{array}
$$
